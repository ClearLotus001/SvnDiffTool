...
const RevisionDatePicker = memo(({
  ...
  const initialMonthKey = value ? value.slice(0, 7) : todayValue.slice(0, 7);
  const initialMonthMeta = parseMonthKey(initialMonthKey) ?? {
...
const RevisionPicker = memo(({
  ...
  const specialOptions = useMemo(() => options.filter((option) => option.kind !== 'revision'), [options]);
  ...
  const filteredSpecialOptions = useMemo(
    () => {
      ...
      return !deferredSearchQuery
        ? specialOptions
        : specialOptions.filter((option) => buildRevisionSearchText(option).includes(deferredSearchQuery));
    },
    [activeDateFilter, deferredSearchQuery, specialOptions],
  );
  ...
  const visibleCount = filteredSpecialOptions.length + filteredRevisionOptions.length;
        ...
        <div style={{ position: 'absolute', top: 'calc(100% + 10px)', ...panelAlignStyle, zIndex: 72, width: UI.panelWidth, borderRadius: UI.panelRadius, border: `1px solid ${T.border}`, background: `linear-gradient(180deg, ${T.bg1} 0%, ${T.bg0} 100%)`, boxShadow: `0 24px 48px -28px ${T.border2}`, overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', padding: '10px 12px 12px' }}>
            <div ref={listRef} onScroll={handleListScroll} style={{ maxHeight: UI.listMaxHeight, overflowY: 'auto', overflowX: 'hidden', border: `1px solid ${T.border}`, borderRadius: 14, background: T.bg1, scrollbarWidth: 'thin' }}>
              ...
              {filteredSpecialOptions.length > 0 && filteredRevisionOptions.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderBottom: `1px solid ${T.border}`, background: `linear-gradient(180deg, ${T.bg2} 0%, ${T.bg1} 100%)` }}>
                  <span aria-hidden="true" style={{ width: 8, height: 8, borderRadius: 2, background: T.acc2, boxShadow: `0 0 0 4px ${T.acc2}14` }} />
                  <span style={{ color: T.t2, fontSize: UI.metaSize, fontWeight: 700, fontFamily: FONT_UI, textTransform: 'uppercase', letterSpacing: 0.4 }}>
                    {t('revisionPickerHistoryGroup')}
                  </span>
                </div>
              )}
