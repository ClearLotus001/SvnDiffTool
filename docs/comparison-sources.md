...
The renderer prefers `window.versora`; `window.svnDiff` remains as a compatibility alias for existing TortoiseSVN launches and test harnesses. New settings use `versora.*` keys and fall back to the legacy `svn-excel-diff-tool.*` keys on first read.
